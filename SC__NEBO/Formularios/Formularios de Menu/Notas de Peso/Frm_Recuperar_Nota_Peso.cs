...
{
    public partial class Frm_Recuperar_Nota_Peso : Form
    {
        Clases.DB db = new Clases.DB();
        Clases.Asistente a = new Clases.Asistente();

        public Frm_Recuperar_Nota_Peso()
        {
            InitializeComponent();
        }

        private void Frm_Recuperar_Nota_Peso_Load(object sender, EventArgs e)
        {
            GetNotaPeso();
        }
        ...
        private void GetNotaPeso()
        {
            string query = "A.ID_NOTA, B.NOMBRE, C.UBICACION + ' ' + C.MUNICIPIO AS UBICACION, A.FECHA_NOTA_PESO, E.ESTADO, A.ESTADO AS ESTADO_NOTA, A.PESO_BRUTO," +
                ...
                " ESTADO_CAFE E ON(A.ID_ESTADO_CAFE = E.ID) INNER JOIN COSECHAS F ON(A.ID_COSECHA = F.ID_COSECHA) WHERE A.ESTADO = 'EN PROCESO'";
        ...
        private void btnRecuperar_Nota_Click(object sender, EventArgs e)
        {
            if (DgvData.Rows.Count > 0)
            {
                string codnotapeso = DgvData.CurrentRow.Cells[0].Value.ToString();
                string nombre = DgvData.CurrentRow.Cells[1].Value.ToString();
                string msg = "¿DESEA RECUPERAR LA NOTA DE PESO " + codnotapeso + " DEL SOCIO " + nombre + " ?";

                if (a.Pregunta(msg) == true)
                {
                    if (db.Recuperar_Nota("NOTA_DE_PESO", "ID_NOTA", codnotapeso) > 0)
                    {
                        a.Aprueba("!LA NOTA SE RECUPERO CORRECTAMENTE");
                        GetNotaPeso();
                    }
                }
